...
#graph by percent
all_totals %>% 
  ggplot(aes(platform,percent, fill = type)) + geom_bar(stat ="identity", position = "dodge") + geom_text(aes(label = paste(round(percent), "%")), vjust = -0.5, size = 3, position=position_dodge(1))
...
```{r}
#imdb vs votes
all_titles %>% ggplot() + aes(imdb_score ,tmdb_popularity, color = type) + geom_point( alpha = .4, na.rm = T, position = 'jitter') + facet_wrap(~ platform) + scale_y_log10()

#tmdb vs popularity
all_titles %>% ggplot() + aes(tmdb_score ,tmdb_popularity, color = type) + geom_point( alpha= .4, na.rm = T, position = 'jitter') + facet_wrap(~ platform) + scale_y_log10()

#imdb vs tmdb
all_titles %>% ggplot() + aes(tmdb_score ,imdb_score, color = type) + geom_density_2d(na.rm = T, position = 'jitter')+ facet_wrap(~ platform)
```
#imdb vs tmdb
...
all_titles %>% ggplot() + aes(imdb_score, fill = type) + geom_density(alpha = 0.4)

#tmbd
all_titles %>% ggplot() + aes(tmdb_score, fill = type) + geom_density(alpha = 0.4)+facet_wrap(~ platform)

all_titles %>% ggplot() + aes(tmdb_score, fill = type) + geom_density(alpha = 0.4)
...
all_titles %>% 
  ...
  summarise(count = n())
all_titles%>% strsplit(genres, ",") %>% group_by(genres) %>% summarise(n())
